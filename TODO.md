...
- [ ] **Behavioral install (install-smoke `install` job)** — boot ISO → drive Calamares over VNC → install to blank disk → boot the INSTALLED disk. IN PROGRESS: recon confirmed the live GNOME desktop; iterating on GUI automation (launch via Activities search). This is what finally exercises partition→unpackfs→grub end-to-end.
- [ ] **UEFI-capable ISO** — Ubuntu's fork can't do EFI at all; real modern hardware (the 5090 box) may need it. Path: migrate to Debian's modern live-build (live-boot stack, different casper wiring + unpackfs path) — a deliberate project, not a flag.
