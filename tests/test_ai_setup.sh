...
# and no real GPU/LM Studio is touched. Config goes to a throwaway XDG_CONFIG_HOME.
...
make_env() {
  ...
  local s
  for s in 01-install-lmstudio 02-preload-models 03-install-comfyui 04-download-image-models; do
    { printf '#!/usr/bin/env bash\n'; printf 'echo "RAN %s $*"\n' "$s"; } > "$root/ai/setup/$s.sh"
    chmod +x "$root/ai/setup/$s.sh"
  done
  echo "$root"
}
run_setup() { local root="$1"; shift; XDG_CONFIG_HOME="$root/cfg" "$root/ai/bin/distro-ai-setup" "$@"; }
...
assert_contains "--install runs LM Studio install" "$out" "RAN 01-install-lmstudio"
